...
// End Session (deletes everything from memory). Cabeza de Vaca's Journey is
// single-role solo, so students start on their own as soon as they're approved.
...
export default function CommandCenter() {
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Cabeza de Vaca’s Journey · everyone walks as Cabeza de Vaca · TEKS 7.2B, 7.2A</p>
  ...
  const cls = roster?.classAccuracy?.cabeza || { count: 0, average: 0 };
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Cabeza de Vaca’s Journey · session-only data · download the PDF before you end</p>
        </div>
      ...
      <div className="card">
        ...
        <div className="class-summary">
          ...
          <div className="summary-stat">
            <div className="big-number">{cls.count}</div>
      ...
      <footer className="app-footer">7th Grade Texas History · Cabeza de Vaca’s Journey · TEKS 7.2B, 7.2A</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  const doc = new jsPDF();
  const BRAND = [23, 100, 107]; // deep gulf teal

  doc.setFontSize(16);
  doc.text('Cabeza de Vaca’s Journey — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  autoTable(doc, {
    ...
    head: [['Class summary — The Castaways', '']],
    body: [
      ['Students completed', String(cls.count)],
      ['Average accuracy', cls.count ? `${cls.average}%` : '—'],
    ],
    styles: { fontSize: 9 },
    headStyles: { fillColor: BRAND },
  });
  ...
  doc.text('7th Grade Texas History · Cabeza de Vaca’s Journey · TEKS 7.2B, 7.2A', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`cabeza-de-vaca-journey_${roster.joinCode}_${date}.pdf`);
}
