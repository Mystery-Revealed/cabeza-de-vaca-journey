...
export class GameManager {
  ...
  createSession({ gameId = 'cabeza-de-vaca', pin, requireApproval = config.requireApprovalDefault }) {
  ...
  _matchBeginPayload(game, match, side, opponentName) {
    ...
    // Single-role solo games (e.g. Hold the Line) have no rival side, so
    // otherSide() is undefined — rivalMeters is null in that case.
    const rival = otherSide(game, side);
    return {
  ...
  _afterMove(session, game, match, side, res) {
    ...
    if (match.solo && res.chapterDone) {
      const aiSide = otherSide(game, side);
  ...
  _completeMatch(session, game, match) {
    ...
    for (const [side, sid] of players) {
      ...
      const rivalSide = otherSide(game, side);
